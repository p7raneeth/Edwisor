...
cv = lapply(folds, function(x){
  ...
  cnt <<- cnt + 1
  write.table(pred_file[,c(1:4,3508,5:3507)],paste0('prediction_file_Folds_0',cnt,'.csv'),sep = ',',row.names = FALSE,append = TRUE)
  cm = table(test_set$Class,class_predicted)
  write.table(cm,paste0('confusion_matrix_Folds_0',cnt,'.csv'),sep = ',',append = TRUE)
